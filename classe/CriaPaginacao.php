...
class criaPaginacao extends MinhaConexao //Indicar o arquivo que será herdado
{
	...
	public function geraNumeros()
	{
		...
		if ($this->ida > 1) {
			echo "<li class='link-underline link-underline-opacity-0'><a href=\"$this->fileName&pg=$this->pagAnt\" title=\"$this->pagAnt\">Anterior</a>\n<li>";
		}
		...
		while (($this->lnk_impressos < $this->numeroPaginas) and ($this->lnk_impressos < $this->maxLink)) {
			...
			if ($this->pagAtual == $this->lnk_impressos) {
				echo "<li class='link-underline link-underline-opacity-0'>$this->lnk_impressos \n</li>";
				//mostra os números das 
			} else {
				echo "<li class='link-underline link-underline-opacity-0'><a href=\"$this->fileName&pg=$this->lnk_impressos\" title=\"$this->lnk_impressos\">$this->lnk_impressos</a></li>\n";
			}
		}
		// mostra o link PRÓXIMO >>
		if ($this->registroFinal < $this->resultadoTotal) {
			echo "<li class='link-underline link-underline-opacity-0'><a href=\"$this->fileName&pg=$this->proxPag\" title=\"$this->proxPag\">Pr&oacute;ximo</a></li>\n";
		}
	}
